...
import { createBroadcaster, createEvent } from './broadcaster.js';
...
export const createWebServer = (deps) => {
        ...
        broadcaster = createBroadcaster({
            onClientMessage: async (clientId, message) => {
                if (message.type === 'subscribe_terminal' && message.sessionName) {
                    ...
                    const { sessionId, command } = message.payload;
                    const session = sessionManager.getByName(sessionId);
                    if (session) {
                        switch (command) {
                            case 'approve':
                    ...
                    const { since, sessionName } = message.payload;
    ...
    const getBroadcaster = () => {
        if (!broadcaster)
            throw new Error('Web server not started');
        return broadcaster;
    };
...
// ──────────────────────────────────────
function wireSessionEvents(sessionManager, broadcaster, terminalStream, getTunnelUrl) {
    // Map to store ACP parsers per session
    const acpParsers = new Map();
    ...
    sessionManager.on('session:stopped', (data) => {
        // Clean up ACP parser
    ...
    sessionManager.on('approval:needed', async (session) => {
        broadcaster.broadcast(createEvent('approval_needed', {
            ...
            if (broadcaster.isUserOffline()) {
                ...
                if (notifier.isEnabled()) {
                    ...
                    const clickUrl = tunnelUrl
                        ...
                        : undefined;
                    await notifier.sendNotification(`OpenSofa: Approval needed in ${session.name}`, session.pendingApproval?.command ? `Command: ${session.pendingApproval.command}` : 'A command requires your approval', clickUrl);
                }
            }
        }
        catch (err) {
    ...
    sessionManager.on('approval:cleared', (session) => {
        broadcaster.broadcast(createEvent('approval_cleared', { sessionName: session.name }, session.name));
    });
...
function storeClientSocket(clientId, ws) {
    clientSockets.set(clientId, ws);
}
function removeClientSocket(clientId) {
    clientSockets.delete(clientId);
}
function getSocketForClient(clientId) {
