...
import { safeGitExec, safeTmuxExec } from './utils/safe-shell.js';
 ...
 */
function agentClient(port) {
    return new AgentAPIClient(port);
}
export class SessionManager extends EventEmitter {
    sessions = new Map(); // key: session name
    usedPorts = new Set();
    startupProcesses = new Map();
    creatingSessions = new Set(); // guards against duplicate in-flight creates
    config;
    classifier;
    agentRegistry;
    ...
    onStateChanged = null;
    webUrlProvider = null;
    constructor(config, classifier, agentRegistry) {
        super();
        this.config = config;
     ...
     */
    async createSession(name, dir, agent, model = '') {
     ...
     */
    async spawnAgentAPI(port, agent, model, workDir) {
    ...
    async healthCheck(port, timeoutMs) {
        try {
            await agentClient(port).waitUntilReady(timeoutMs, 500, () => this.isStartupProcessAlive(port));
        }
        finally {
     ...
     */
    async sendToAgent(session, text) {
     ...
     */
    async sendToAgentInternal(session, text) {
        const client = agentClient(session.port);
        try {
     ...
     */
    async uploadMedia(session, buffer, fileName, mimetype, caption) {
     ...
     */
    async handleSessionCommand(session, command) {
        session.lastActivityAt = Date.now();
        switch (command.cmd) {
            case 'stop':
     ...
     */
    async sendApproval(session, response) {
        if (!session.pendingApproval) {
     ...
     */
    async switchSessionAgent(session, value) {
     ...
     */
    async switchSessionDirectory(session, value) {
        ...
        await sleep(1000);
        try {
            const pid = await this.spawnAgentAPI(previous.port, previous.agentType, previous.model, newWorktree.workDir);
            ...
            try {
                const restorePid = await this.spawnAgentAPI(previous.port, previous.agentType, previous.model, previous.workDir);
                await this.healthCheck(previous.port, this.getStartupTimeoutMs(previous.agentType));
                session.pid = restorePid;
     ...
     */
    async setSessionSetting(name, key, value) {
        const session = this.sessions.get(name);
        if (!session) {
            throw new Error(`Session '${name}' not found.`);
        }
     ...
     */
    async recoverSessions(persisted) {
        ...
        fc.on('event', (event) => {
            ...
            if (event.type === 'approval') {
                ...
                if (session.autoApprove) {
                    void this.sendAutoApproval(session, command).catch((autoErr) => {
    ...
    clearRuntime(session) {
        ...
        if (session.feedbackController) {
            session.feedbackController.disconnect();
            session.feedbackController = null;
        }
    }
    async sendAutoApproval(session, command) {
        try {
    ...
    async restartSession(name) {
        ...
        const portUsedByAnotherSession = Array.from(this.sessions.values()).some(s => s.name !== session.name && s.status === 'active' && s.port === restartPort);
    ...
    setPendingApproval(session, command) {
        this.clearPendingApproval(session);
        const pending = {
            detectedAt: Date.now(),
            command,
        ...
        if (this.notifier) {
            void this.notifier.sendNotification(`OpenSofa Task Stalled`, `Agent '${session.name}' requires human input for: ${command || 'a shell command'}`, `/?session=${session.name}`);
        }
    }
    ...
    async handleApprovalTimeout(session) {
