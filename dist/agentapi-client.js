...
export class AgentAPIClient {
     ...
     */
    async waitUntilReady(timeoutMs = 30_000, intervalMs = 500, isProcessAlive) {
        const deadline = Date.now() + timeoutMs;
        let lastError = '';
        while (Date.now() < deadline) {
            if (isProcessAlive && !isProcessAlive()) {
                throw new AgentAPIError('Health check failed: AgentAPI process exited before becoming ready.', 0, 'AgentAPI exited during startup. Check agent installation, API keys, and first-time setup.');
            }
            try {
                const res = await fetch(`${this.baseUrl}/status`, {
        ...
        throw new AgentAPIError(`Health check timeout after ${timeoutMs}ms${lastError ? ` (last error: ${lastError})` : ''}`, 0, 'AgentAPI did not become ready in time. Check CLI auth/setup and try again.');
    }
    // ── Internals ─────────────────────────────────────────
     ...
     */
    classifyHttpError(status, msgType) {
...
export class AgentAPIError extends Error {
    httpStatus;
    userFriendlyMessage;
    constructor(message, httpStatus = 0, userFriendlyMessage) {
        super(message);
        this.httpStatus = httpStatus;
        this.userFriendlyMessage = userFriendlyMessage;
        this.name = 'AgentAPIError';
    }
    static fromFetchError(err) {
        const raw = String(err);
        const isConnectivity = /fetch failed|ECONNREFUSED|ENOTFOUND|timed out|AbortError/i.test(raw);
        return new AgentAPIError(raw, 0, isConnectivity
            ? 'AgentAPI not responding. Try /restart or /stop then create a new session.'
            : undefined);
    }
}
//# sourceMappingURL=agentapi-client.js.map