...
function checkDirectory(path: string, name: string): CheckResult {
  if (existsSync(path)) {
    return {
      name,
      status: 'ok',
 ...
 */
export async function runPrerequisitesCheck(agentRegistry: AgentRegistry): Promise<string> {
  ...
  for (const r of results.filter(r => 
    ['Node.js', 'git', 'tmux', 'AgentAPI'].includes(r.name)
  )) {
  ...
  lines.push('\n*Agents:*');
  for (const r of results.filter(r => 
    !['Node.js', 'git', 'tmux', 'AgentAPI', 'Config directory'].includes(r.name)
  )) {
    lines.push(r.message);
  }
