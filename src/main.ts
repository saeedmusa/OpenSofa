...
async function main(): Promise<void> {
  ...
  resourceMonitor.on('session:unhealthy', async (name: string, reason: string) => {
    log.warn('Session unhealthy', { name, reason });
    await notifier.sendNotification(
      'Session Unhealthy',
      `[${name}] ⚠️ Health check failed: ${reason}`,
      `/?session=${name}`
    );
  });

  resourceMonitor.on('session:idle', async (name: string, idleMs: number) => {
    const mins = Math.round(idleMs / 60000);
    log.info('Session idle detected', { name, idleMs });
    await notifier.sendNotification(
      'Session Idle',
      `[${name}] 💤 Session idle for ${mins} minutes`,
      `/?session=${name}`
    );
  });

  resourceMonitor.on('resources:critical', async (stats: ResourceStats) => {
    log.error('Resources critical', { ...stats });
    await notifier.sendNotification(
      '⚠️ Critical Resources',
      `CPU: ${stats.cpu}% | Free RAM: ${Math.round(stats.freeMemMB)}MB`
    );
  });

  // Auto-cleanup idle sessions when resources are critical
  resourceMonitor.on('session:cleanup', async (name: string, reason: string) => {
    try {
      log.warn('Auto-cleanup triggered', { name, reason });
      await sessionManager.stopSession(name);
      await notifier.sendNotification(
        'Session Auto-Stopped',
        `🧹 Auto-stopped idle session "${name}" due to ${reason}`
      );
    } catch (err) {
 ...
 */
function emergencyShutdown(err: Error): void {
 ...
 */
async function handleRevokeCommand(): Promise<void> {
