...
const CONTROL_PATTERNS: Array<{
  regex: RegExp;
  parse: (match: RegExpMatchArray, defaultAgent: AgentType) => ControlCommand;
}> = [
  ...
  // /new <name> <dir> [agent] [model]
  {
    ...
    parse: (match, defaultAgent) => ({
      ...
      name: match[1]!,
      dir: match[2]!,
      agent: (match[3] as AgentType) ?? defaultAgent,
      model: match[4] ?? '',
    }),
  },
  // /stop <name>
 ...
 */
const SESSION_PATTERNS: Array<{
  regex: RegExp;
  parse: () => SessionCommand;
}> = [
