...
export const createUploadRoutes = (deps: UploadRoutesDeps): Hono => {
    const app = new Hono();
    ...
    app.post('/:name/upload', async (c) => {
        ...
        try {
            ...
            if (!ALLOWED_TYPES.has(file.type)) {
                return c.json(
                    error(`Unsupported file type: ${file.type}. Allowed: JPEG, PNG, WebP`, 'INVALID_TYPE'),
                    400,
                );
            }
            ...
            if (file.size > MAX_FILE_SIZE) {
                return c.json(
                    error(`File too large (${(file.size / 1024 / 1024).toFixed(1)}MB). Max: 2MB`, 'FILE_TOO_LARGE'),
                    400,
                );
            }
