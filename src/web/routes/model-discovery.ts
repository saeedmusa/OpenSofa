 ...
 */
function parseAgentFilter(agentsParam: string | undefined): AgentType[] | undefined {
  if (!agentsParam) return undefined;

  const agents = agentsParam
    .split(',')
    .map(a => a.trim().toLowerCase() as AgentType)
    .filter((a): a is AgentType => Boolean(a));
 ...
 */
export function createModelDiscoveryRoutes(): Hono {
  ...
  app.get('/discover', async (c) => {
    ...
    let result: ModelDiscoveryResult | undefined;
